...
class GoogleFormatter {
  ...
  outputGoogleStatements = async (query: string) => {
    const googleData = await fetchGoogleData(
      `https://factchecktools.googleapis.com/v1alpha1/claims:search?&query=${query}&key=AIzaSyBwIZ8rP8aE8eD9O4s9HYlozrUEtCc_Olo`
    );
...
// const googleFormatter = new GoogleFormatter();
// googleFormatter.outputGoogleStatements(
//   "I’m a Celebrity: MPs and peers voting for Hancock to do ‘grim’ tasks, says minister"
// );
// googleFormatter.outputGoogleResults(
//   "I’m a Celebrity: MPs and peers voting for Hancock to do ‘grim’ tasks, says minister"
// );
